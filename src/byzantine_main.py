"""
Federated Learning with Byzantine attack simulation.

Attack type : data poisoning — byzantine clients mix abnormal traffic into
              their training set, forcing the model to reconstruct attack
              patterns as normal.

The script loops over byzantine ratios 10 % → 50 % (step +10 %) automatically.
Each ratio writes to its own result directory so all runs are preserved.

Usage:
  python byzantine_main.py --file Configuration/scen2-nba-iot-byzantine-10clients-iid.json
  python byzantine_main.py --file Configuration/scen2-nba-iot-byzantine-10clients-noniid.json
"""
...
# Attack settings — fixed, ratio is the only variable
ATTACK_TYPE     = "data_poisoning"
POISONING_RATIO = 0.3             # 30 % of train samples replaced with abnormal
...
def build_poisoned_loader(proc_train, proc_abnormal, batch_size):
    """Replace POISONING_RATIO of training samples with abnormal samples."""
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------ #
    # Load data once — reused across all ratio iterations                  #
    # ------------------------------------------------------------------ #
    with open(config_file) as f:
        config = json.load(f)
    ...
    for byzantine_ratio in BYZANTINE_RATIOS:
        ...
        for model_type in ["autoencoder", "hybrid"]:
            for run in range(num_runs):
                ...
                logging.info("byz=%d%%  run=%d  model=%s  byzantine=%s",
                             byz_pct, run, model_type, byz_names)
                ...
                for rnd in range(num_rounds):
                    ...
                    total_samples    = sum(len(c["train_loader"].dataset) for c in selected_clients)
                    ...
                    logging.info("byz=%d%%  rnd=%d/%d  val_loss=%.6f",
                                 byz_pct, rnd + 1, num_rounds, global_aggregator.val_loss)
